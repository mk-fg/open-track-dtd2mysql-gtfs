...
class GTFSDB:
	...
	def print_train_info( self,
		...
		if db_cif and train_uid:
			...
			sched_query = f'''
				SELECT
					s.train_uid, s.id, stp_indicator stp,
					runs_from a , runs_to b, bank_holiday_running always,
					CONCAT(monday, tuesday, wednesday, thursday, friday, saturday, sunday) days
					{'-- , COALESCE(ps.crs_code, t.crs_code) crs_code' if not z else ', location crs_code'}
					-- , public_arrival_time ts_arr, public_departure_time ts_dep, activity
				FROM {db_cif}.{z}schedule s
				-- LEFT JOIN {db_cif}.{z}stop_time st ON st.{z}schedule = s.id
				{( f'-- LEFT JOIN {db_cif}.physical_station ps ON ps.tiploc_code = st.location'
					f' LEFT JOIN {db_cif}.tiploc t ON t.tiploc_code = st.location' ) if not z else ''}
				WHERE
					train_uid IN ({train_uid_tuple})
					{'-- AND (st.id IS NULL OR COALESCE('
						'ps.crs_code, t.crs_code) IS NOT NULL)' if not z else ''}
				ORDER BY
